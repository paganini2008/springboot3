...
import com.fred.api.UserManagementApplication;
...
public class UserControllerTest {
    ...
    public void testRegisterUser() throws Exception {
        ...
        String contentStr = mockMvc.perform(post("/user/register").content(jsonStr).contentType(
                MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
        ApiResult<UserVo> apiResult = objectMapper.readValue(contentStr, new TypeReference<ApiResult<UserVo>>() {
        });
    ...
    public void testUpdateUser() throws Exception {
        ...
        String contentStr = mockMvc.perform(put("/user/update").content(jsonStr).contentType(
                MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andReturn().getResponse().getContentAsString();
        ApiResult<UserVo> apiResult = objectMapper.readValue(contentStr, new TypeReference<ApiResult<UserVo>>() {
        });
    ...
    public void testGetUserByEmail() throws Exception {
        String contentStr = mockMvc.perform(get("/user/by").param("email", "tester1@test.com")).andExpect(
    ...
    public void testDeleteUserByEmail() throws Exception {
        String contentStr = mockMvc.perform(delete("/user/by").param("email", "tester1@test.com")).andExpect(
                status().isOk()).andReturn().getResponse().getContentAsString();
        ApiResult<Boolean> apiResult = objectMapper.readValue(contentStr, new TypeReference<ApiResult<Boolean>>() {
        });
    ...
    public void testGetUserByEmailIfNotExisted() throws Exception {
        String contentStr = mockMvc.perform(get("/user/by").param("email", "tester1@test.com")).andExpect(
                status().isNotFound()).andReturn().getResponse().getContentAsString();
